...
class threadClass:
    ...
    def run(self):
        ...
        collection_name = self.scrapper_object.getReviewsToDisplay(expected_review=self.expected_review,
                                                                   searchString=self.searchString, username='upend',
                                                                   password='upend',
...
def index():
    if request.method == 'POST':
        ...
        try:
            ...
            mongoClient = MongoDBManagement(username='upend', password='upend')
    ...
    try:
        ...
        if collection_name is not None:
            scrapper_object = FlipkratScrapper(executable_path=ChromeDriverManager().install(),
                                               chrome_options=chrome_options)
            mongoClient = MongoDBManagement(username='upend', password='upend')
            rows = mongoClient.findAllRecords(db_name="FlipkartScrapper", collection_name=collection_name)
